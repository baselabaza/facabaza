...
os.execute('chmod +x Abaza;chmod +x Abaza;./Abaza')
end
...
bot = Fx.Abazabots.set_config{api_id=1846213,api_hash='c545c613b78f18a30744970910124d53',session_name=Abaza,token=Token}
function var(value)
...
function Dev(msg) 
if Redis:sismember(Abaza.."Dev",msg.sender_id.user_id) or tonumber(msg.sender_id.user_id) == tonumber(Sudo_Id) then
ok = true
else
ok = false
end
...
if tonumber(data.sender_id.user_id) == tonumber(Abaza) then
return false
end
end
function ChannelJoin(id)
JoinChannel = true
local chh = Redis:get(Abaza.."chfalse")
if chh then
...
if data.sender_id.user_id == tonumber(Abaza) then
return false
end
if Redis:get(Abaza.."chsource") then
chsource = Redis:get(Abaza.."chsource")
else
chsource = "otlop12"
end
...
Redis:del(Abaza..data.sender_id.user_id.."bottoken")
Redis:del(Abaza..data.sender_id.user_id.."botuser")
Redis:del(Abaza..data.sender_id.user_id.."make:bot")
...
Redis:del(Abaza.."free:bot")
send(data.chat_id,data.id,'☤ تم تفعيل الوضع المجاني ',"md",true)  
end
if text == "☤ تعطيل الوضع المجاني" then 
Redis:set(Abaza.."free:bot",true)
send(data.chat_id,data.id,'☤ تم تعطيل الوضع المجاني ',"md",true)  
end
-----تشغيل البوتات ---
if text and Redis:get(Abaza..data.sender_id.user_id.."run:bot") then
Redis:del(Abaza..data.sender_id.user_id.."run:bot")
Redis:del(Abaza.."screen:on")
Redis:del(Abaza.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
if folder:match('@[%a%d_]') then
Redis:sadd(Abaza.."bots:folder",folder:gsub("@",""))
end
end
if not Redis:sismember(Abaza.."bots:folder",userbot) then
...
Redis:sadd(Abaza.."screen:on",screen)
end
local list = Redis:smembers(Abaza..'screen:on')
...
Redis:set(Abaza..data.sender_id.user_id.."run:bot",true)
send(data.chat_id,data.id,"☤ ارسل معرف البوت ليتم تشغيله")
return false 
end
---ايقاف البوتات
if text and Redis:get(Abaza..data.sender_id.user_id.."stop:bot") then
Redis:del(Abaza..data.sender_id.user_id.."stop:bot")
Redis:del(Abaza.."screen:on")
Redis:del(Abaza.."bots:folder")
userbot = text:gsub("@","")
for folder in io.popen('ls'):lines() do
...
Redis:set(Abaza..data.sender_id.user_id.."stop:bot",true)
send(data.chat_id,data.id,"☤ ارسل معرف البوت ليتم ايقافه")
return false 
end
--الاشتراك الاجباري 
if Redis:get(Abaza.."ch:addd"..data.sender_id.user_id) == "on" then
Redis:set(Abaza.."ch:addd"..data.sender_id.user_id,"off")
...
Redis:del(Abaza.."chfalse")
Redis:set(Abaza.."chfalse",ch)
Redis:del(Abaza.."ch:admin")
Redis:set(Abaza.."ch:admin",da.result.invite_link)
else
send(data.chat_id,data.id,'☤ المعرف خطأ او البوت ليس مشرف في القناه ',"md",true)  
end
end
if text == "☤ تفعيل الاشتراك الاجباري" then
Redis:set(Abaza.."ch:addd"..data.sender_id.user_id,"on")
...
if text and Redis:get(Abaza..data.sender_id.user_id.."make:bot") == "devuser" then
...
local bottoken = Redis:get(Abaza..data.sender_id.user_id.."bottoken")
local botuser = Redis:get(Abaza..data.sender_id.user_id.."botuser")
...
Redis:sadd(Abaza.."userbots",botuser)
Redis:del(Abaza..data.sender_id.user_id.."bottoken")
Redis:del(Abaza..data.sender_id.user_id.."botuser")
Redis:del(Abaza..data.sender_id.user_id.."make:bot")
...
if text and Redis:get(Abaza..data.sender_id.user_id.."make:bot") == "token" then
...
if Redis:sismember(Abaza.."userbots",botuser) then
send(data.chat_id,data.id, "\n☤ عذرا هذا البوت مصنوع بالفعل","md",true)  
return false 
end 
Redis:set(Abaza..data.sender_id.user_id.."botuser",botuser)
Redis:set(Abaza..data.sender_id.user_id.."bottoken",text)
Redis:set(Abaza..data.sender_id.user_id.."make:bot","devuser")
...
Redis:set(Abaza..data.sender_id.user_id.."make:bot","token")
...
list = Redis:smembers(bot_id.."ChekBotAdd") 
lt = Redis:smembers(bot_id.."Num:User:Pv") 
Redis:incrby(Abaza.."All:gp:st",tonumber(#list))
Redis:incrby(Abaza.."All:pv:st",tonumber(#lt))
end
end
end
...
Redis:sadd(Abaza..'3ddbots',folder)
end
end
...
Redis:del(Abaza.."fake")
Redis:del(Abaza..'BaN:In:Tuasl')  
send(data.chat_id,data.id,"• done")
end
...
-------delete 
if text and Redis:get(Abaza..data.sender_id.user_id.."make:bot") == "del" then
...
Redis:srem(Abaza.."userbots",text:gsub("@",""))
send(data.chat_id,data.id, "\n☤ تم حذف البوت بنجاح","md",true)  
return false 
end 
if text == "☤ حذف بوت" then
Redis:set(Abaza..data.sender_id.user_id.."make:bot","del")
...
devbot = x[3]
bottoken = x[4]
Redis:sadd(Abaza..'3ddbots',botuser..' » '..devbot)
end
end
end
local list = Redis:smembers(Abaza..'3ddbots')
...
----end--3dd
if text and Redis:get(Abaza..data.sender_id.user_id.."setchannel") then
if text == "الغاء" or text == '☤ الغاء الامر' then   
Redis:del(Abaza..data.sender_id.user_id.."setchannel")
send(data.chat_id,data.id, "\n☤ تم الغاء تعيين قناه السورس","md",true)  
return false 
end 
if text:match("@(.*)") then
local ch = text:match("@(.*)")
Redis:set(Abaza.."chsource",ch)
...
Y = Y + 1
Redis:sadd(Abaza..'total',v)  
end
end
end
if text == "جلب نسخه" then
local UsersBot = Redis:smembers(Abaza.."total")
local Get_Json = '{"BotId": '..Abaza..','  
if #UsersBot ~= 0 then 
...
for k,v in pairs(list) do
https.request("https://api.telegram.org/bot"..bottoken.."/sendmessage?chat_id="..v.."&text="..URL.escape(Redis:get(Abaza.."eza3a:pv")).."&parse_mode=Markdown")
end
end
end
end
...
--brodcast
if Redis:get(Abaza..data.sender_id.user_id.."brodcast") then 
if text == "الغاء" or text == '☤ الغاء الامر' then   
Redis:del(Abaza..data.sender_id.user_id.."brodcast") 
send(data.chat_id,data.id, "\n☤ تم الغاء الاذاعه","md",true)  
return false 
end 
local list = Redis:smembers(Abaza.."total") 
...
if text == "☤ اذاعه" then
Redis:set(Abaza..data.sender_id.user_id.."brodcast",true)
send(data.chat_id,data.id,"☤ ارسل الاذاعه الان")
return false 
end
---fwd
if Redis:get(Abaza..data.sender_id.user_id.."brodcast:fwd") then 
if text == "الغاء" or text == '☤ الغاء الامر' then   
Redis:del(Abaza..data.sender_id.user_id.."brodcast:fwd")
...
Redis:set(Abaza..data.sender_id.user_id.."brodcast:fwd",true)
...
chinfo = Redis:get(Abaza.."ch:admin")
var(bot.sendText(msg.chat_id,msg.id,'\n☤ عليك الاشتراك في قناة البوت لاستخذام الاوامر\n\n'..chinfo..''))
return false 
end
if not Redis:get(Abaza.."twsl") then
if msg.sender_id.user_id ~= tonumber(Abaza) then
if Redis:sismember(Abaza..'BaN:In:Tuasl',msg.sender_id.user_id) then
return false 
end
if msg.id then
Redis:setex(Abaza.."Twasl:UserId"..msg.date,172800,msg.sender_id.user_id)
...
if text and Redis:get(Abaza.."free:bot") then
return send(msg.chat_id,msg.id,"☤ الوضع المجاني معطل من قبل مطور الصانع")
end
if text == "/start" then
if not Redis:sismember(Abaza.."total",msg.sender_id.user_id) then
Redis:sadd(Abaza.."total",msg.sender_id.user_id)
end
reply_markup = bot.replyMarkup{type = 'keyboard',resize = true,is_personal = true,
data = {
{
...
send(msg.chat_id,msg.id," ⋆ مرحبا بك في صانع بوتات حمايه المجاني.\n⋆ قم بتنصيب بوت مجاني الان وبحقوقك انت وقناتك اي استفسار @B100Q.","html",true, false, false, true, reply_markup)
return false 
end
---making user
if text and Redis:get(Abaza..msg.sender_id.user_id.."make:bot") then
if text == "☤ الغاء" then
Redis:del(Abaza..msg.sender_id.user_id.."make:bot")
...
if Redis:sismember(Abaza..'fake',botuser) then
...
Informationlua:close()
os.execute('cp -a ./source/. ./@'..botuser..' && cd @'..botuser..' && chmod +x * && screen -d -m -S '..botuser..' ./Run')
Redis:set(Abaza..msg.sender_id.user_id.."my:bot",botuser)
Redis:sadd(Abaza.."userbots",botuser)
Redis:del(Abaza..msg.sender_id.user_id.."make:bot")
...
if text == "☤ صنع بوت" then
if Redis:get(Abaza..msg.sender_id.user_id.."my:bot") then
return send(msg.chat_id,msg.id,"☤ عفوا لديك بوت بالفعل احذفه اولا")
end
Redis:set(Abaza..msg.sender_id.user_id.."make:bot",true)
...
if Redis:get(Abaza..msg.sender_id.user_id.."my:bot") then
local botuser = Redis:get(Abaza..msg.sender_id.user_id.."my:bot")
...
Redis:srem(Abaza.."userbots",botuser)
Redis:del(Abaza..msg.sender_id.user_id.."my:bot")
send(msg.chat_id,msg.id, "\n☤ تم حذف البوت بنجاح","md",true)  
else
...
if data and data.Abazabots and data.Abazabots == "updateNewMessage" then
if tonumber(data.message.sender_id.user_id) == tonumber(Abaza) then
return false
end
Run(data.message,data.message)
elseif data and data.Abazabots and data.Abazabots == "updateMessageEdited" then
local Message_Edit = bot.getMessage(data.chat_id, data.message_id)
if Message_Edit.sender_id.user_id == Abaza then
return false
end
Run(Message_Edit,Message_Edit)
elseif data and data.Abazabots and data.Abazabots == "updateNewCallbackQuery" then
